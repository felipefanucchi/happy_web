import React, { useEffect, useState } from "react";
import { FaWhatsapp } from "react-icons/fa";
import { FiClock, FiInfo } from "react-icons/fi";
import { Map, Marker, TileLayer } from "react-leaflet";

// Styles
import '../styles/pages/orphanage.css';

// Components
import Sidebar from "../components/Sidebar";
import { happyMapIcon } from "../utils/mapIcon";
import api from "../services/api";
import OrphanageType from "../interfaces/Orphanage";
import { useParams } from "react-router-dom";
import OrphanageParams from "../interfaces/OrphanageParams";

const DEFAULT_IMAGE = "https://www.gcd.com.br/wp-content/uploads/2020/08/safe_image.jpg";

export default function Orphanage() {
  const params = useParams<OrphanageParams>();
  const [orphanage, setOrphanage] = useState<OrphanageType | undefined>();
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  useEffect(loadOrphanages, [params.id]);

  function loadOrphanages() {
    api.get(`orphanages/${params.id}`).then(response => {
      setOrphanage(response.data);
    });
  }

  if (!orphanage) {
    return <div className="loader"></div>
  }

  return (
    <div id="page-orphanage">
      <Sidebar />
      <main>
        <div className="orphanage-details">
          <img src={orphanage.images[currentImageIndex].url || DEFAULT_IMAGE} alt={orphanage.name} />

          <div className="images">
            { orphanage.images.map((image, index: number) => (
              <button 
                className={index === currentImageIndex ? 'active' : ''} 
                type="button"
                onClick={() => setCurrentImageIndex(index)}>
                <img src={image.url} alt={orphanage.name} />
              </button>
            )) }
          </div>
          
          <div className="orphanage-details-content">
            <h1>{orphanage.name}</h1>
            <p>{orphanage.about}</p>

            <div className="map-container">
              <Map 
                center={[orphanage.latitude, orphanage.longitude]} 
                zoom={16} 
                style={{ width: '100%', height: 280 }}
                dragging={false}
                touchZoom={false}
                zoomControl={false}
                scrollWheelZoom={false}
                doubleClickZoom={false}
              >
                <TileLayer 
                  url={`https://api.mapbox.com/styles/v1/mapbox/light-v10/tiles/256/{z}/{x}/{y}@2x?access_token=${process.env.REACT_APP_MAPBOX_TOKEN}`}
                />
                <Marker interactive={false} icon={happyMapIcon} position={[orphanage.latitude, orphanage.longitude]} />
              </Map>

              <footer>
                <a target="_blank" rel="noopener noreferrer" href={`https://www.google.com/maps/dir/?api=1&destination=${orphanage.latitude},${orphanage.longitude}`}>Ver rotas no Google Maps</a>
              </footer>
            </div>

            <hr />

            <h2>Instruções para visita</h2>
            <p>{orphanage.instructions}</p>

            <div className="open-details">
              <div className="hour">
                <FiClock size={32} color="#15B6D6" /> Horário de atendimento<br />
                {orphanage.opening_hours}
              </div>

              { orphanage.open_on_weekends ? (
                <div className="open-on-weekends">
                  <FiInfo size={32} color="#39CC83" />
                  Atendemos <br />
                  fim de semana
                </div>
              ) : (
                <div className="open-on-weekends dont-open">
                  <FiInfo size={32} color="#FF669D" />
                  Não atendemos<br />fim de semana
                </div>
              ) }
            </div>

            {/* <button type="button" className="contact-button">
              <FaWhatsapp size={20} color="#FFF" />
              Entrar em contato
            </button> */}
          </div>
        </div>
      </main>
    </div>
  );
}